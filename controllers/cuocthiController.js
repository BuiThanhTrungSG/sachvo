...
const createCuocthi = async (req, res) => {
  ...
  try {
    ...
    const [result] = await conn.query(
      `INSERT INTO cuocthi 
        (tieude, image, socauhoi, batdau, ketthuc, ngaysinh, diachi, sodienthoai, email, cancuoc, noilamviec, xemdiem, xemdapan, daodapan, password)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        tieude,
        image || null,
...
const getCuocthiList = async (req, res) => {
  try {
    const [rows] = await connection.query(
      "SELECT id, tieude, socauhoi, batdau, ketthuc FROM cuocthi ORDER BY id DESC"
    );
    res.json(rows);
  } catch (err) {
...
const updateCuocthi = async (req, res) => {
  ...
  try {
    ...
    // Update main
    await conn.query(
      `UPDATE cuocthi SET tieude=?, image=?, socauhoi=?, batdau=?, ketthuc=?, ngaysinh=?, diachi=?, sodienthoai=?, email=?, cancuoc=?, noilamviec=?, xemdiem=?, xemdapan=?, daodapan=?, password=? WHERE id=?`,
      [
        tieude,
        image || null,
        socauhoi || 0,
